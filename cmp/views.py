...
class ComprasView(SinPrivilegios, generic.ListView):
    ...
    model = ComprasEnc #Modelo a mostrar
...
def compras(request, compra_id = None):
    ...
    prod = Producto.objects.filter(estado=True) #Consulta para obtener todos los productos de estado activo
    form_compras= {}
    contexto= {}

    if request.method == 'GET':
        form_compras = ComprasEncForm()
        enc = ComprasEnc.objects.filter(pk=compra_id).first() #Consulta para obtener las compras respecto al id
        if enc: 
